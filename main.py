...
def output_space_data(mid):
    ...
    while True:
        try:
            ...
            spaceInfo = requests.get(f"https://api.bilibili.com/x/space/wbi/acc/info?mid={mid}", cookies=cookies,
            ...
            Major_err_log()
            print(f"[*{get_date()}*] {traceback.format_exc()}\n")
...
def get_danmu_data():
    ...
    while True:
        try:
            ...
            try:
                d = json.loads(res.text)["data"]["data"]
            except:
                err_log()
                break
